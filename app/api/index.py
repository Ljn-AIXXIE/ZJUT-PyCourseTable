...
def getCourseTableFirstDate(account: Account, host: str = GDJWHost) -> bool:
    xnm = account['acYear']
    xqm: str = ''
    match account['acTerm']:
        case '1': xqm = '3'
    ...
    json = response.json()
    try:
        first_date = json['rqazcList'][0]['rq']
        account['first_date'] = first_date
...
def run(account: Account, host: str = GDJWHost) -> bool:
    able = OAuthCasLogin(account, host)
    able = GDJWLogin(account, host) if able else False
    able = getCourseTable(account, host) if able else False
    able = getCourseTableFirstDate(account, host) if able else False
    return able
...
if __name__ == '__main__':
    ...
    isAble = getCourseTableFirstDate(ac) if isAble else False